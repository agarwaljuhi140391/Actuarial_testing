...
# 		, pcyMONTH_T = {'type': int32, 'alignment':'P', 'numba':'v','shape':2}
		...
		# , pcyBOND_FAV_MIDFIX = {'type': float64, 'alignment':'PCY', 'numba':'j','shape':2,'loop':1,'monthly':True}
...
# 		, 'pEQUITY_TARGET': {'source':'pcEQUITY_TARGET','alignment':'P','func':'sum','loop':1}
	...
	def pcyBOND_FAV_MIDFIX(FAV_monthly,pcREBAL_BUY,pcyBOND_FAV_MIDFIX):
		for mt in range(12):
			if mt == 11:
				pcyBOND_FAV_MIDFIX[mt] = 555
			else:
				pcyBOND_FAV_MIDFIX[mt] = pcREBAL_BUY[0]*FAV_monthly[mt]
	
	def rfpcyBOND_FAV_monthly(pcyBUYSELL_PCTS,FAV_monthly,rfpcyBOND_FAV_monthly):
		for mt in range(12):
	...
	def pcyREALIGN_FAV_monthly(FAV_monthly,rfpcyREALIGN_BUYSELL_PCTS,pcyREBAL_FAV_monthly,pcyREALIGN_FAV_monthly):
		...
		pcyREALIGN_FAV_monthly[11]=FAV_monthly[mt]*rfpcyREALIGN_BUYSELL_PCTS[0]
